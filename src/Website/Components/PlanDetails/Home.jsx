        ...
        <SkeletonLoading loading={loading} error={error} type="detailsGoal">
            ...
            >
              ...
              >
                <img
                  ...
                  src={myplan?.media && myplan.media[0]?.original_url}
                  alt="shoe"
                />
              </div>
            </div>
          ...
          <div className="product-content">
            <MyComponentTitle className="product-title">
              {language === "ar" ? "تفاصيل الخطة" : "myPlan Details"}
            </MyComponentTitle>
            ...
            <div className="product-detail">
              <h1 style={{ padding: "1rem 0" }}>
                {language === "ar" ? "حول هذه الخطة" : "about this myPlan"}:{" "}
              </h1>
              <p>
                {myplan?.plan &&
                  (language === "ar"
                    ? myplan.plan.description_ar
                    : myplan.plan.description)}
              </p>
              <ul>
                <li>
                  <CheckCircleIcon />
                  {language === "ar" ? "الخطة" : "myplan"}:{" "}
                  <span>
                ...
                <li>
                  <CheckCircleIcon />
                  {language === "ar" ? "العضلة المستهدفة" : "muscle"}:{" "}
                  <span>
                    {language === "ar" ? myplan.muscle_ar : myplan.muscle}
                  </span>
                </li>
                <li>
                  <CheckCircleIcon />
                  ...
                  <span>
                    {myplan?.duration} {language === "ar" ? "اسبوع" : "week"}
                  </span>
                </li>
                <li>
                  <div style={{ width: "100%" }} className="card-progress">
                    <div className="progress-wrapper">
                      <p
                        style={{ color: value === "dark" ? "#fff" : "#000" }}
                        className="progress-label"
                      >
                        {language === "ar"
                          ? "نسبة تقدمك في الخطة"
                          : "Your progress on the plan"}
                      </p>
                      ...
                      >
                        {(myplan?.totalRate ? myplan?.totalRate : 0) + "%"}
                      </span>
                    </div>
