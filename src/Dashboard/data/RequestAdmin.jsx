...
import { ActGetRequestAdmin, ActActiveAdmin, ActNotActiveCoachAndAdmin } from "../../Redux/Dashboard/Admin/AdminSlice";
...
const RequestAdmin = () => {
  ...
  const columns = [
    { field: "id", headerName: headerID },
    {
      ...
      renderCell: (Radmin) => <Link to={"/dashboard/user/" + Radmin?.row.id}>{Radmin?.row.name}</Link>,
    },
    {
      field: "why_admin",
      headerName: Why,
      flex: 1,
      cellClassName: "name-column--cell",
      renderCell: (Radmin) => <Link to={"/dashboard/goal/" + Radmin?.row.id}>{Radmin?.row.why_admin}</Link>,
    },
    {
      field: "event",
  ...
  const HandelDestroy = useCallback((id) => {
      dispatch(ActNotActiveCoachAndAdmin(id)).unwrap().then(()=>{
  ...
  return (
    <Box m="20px">
      <Box display="flex" justifyContent="space-between" alignItems="center">
        <Header
          title={language ==='en' ? "Request Admins" : "طلبات الترقية الى ادمن"}
          subtitle={language ==='en' ?"List of Data Request Admin Table" :"سجلات المستخدمين"}
        />
      </Box>
