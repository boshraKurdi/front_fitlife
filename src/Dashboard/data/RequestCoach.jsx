...
import { ActGetRequestCoach, ActActiveCoach, ActNotActiveCoachAndAdmin } from "../../Redux/Dashboard/Admin/AdminSlice";
...
const RequestCoach = () => {
  ...
  const columns = [
    { field: "id", headerName: headerID },
    {
      ...
      renderCell: (Rcoach) => <Link to={"/dashboard/user/" + Rcoach?.row.id}>{Rcoach?.row.name}</Link>,
    },
    {
      field: "description",
      ...
      flex: 1,
      cellClassName: "name-column--cell",
      renderCell: (Rcoach) => <Link to={"/dashboard/goal/" + Rcoach?.row.id}>{Rcoach?.row.description}</Link>,
    },
    {
      field: "event",
...
function HandelUpdateRquest(id) {
  ...
  return (
    <Box m="20px">
      ...
      <Header
          title={language ==='en' ? "Request Coachs" : "طلبات الترقية الى لمدرب"}
